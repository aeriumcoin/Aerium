...
{
    // Convert Aerium:// to Aerium:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Aerium://"))
    {
        uri.replace(0, 12, "Aerium:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Aerium.lnk";
}
...
{
    return GetAutostartDir() / "Aerium.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Aerium-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Aerium-qt [" + tr("command-line options") + "]                     " + "\n";
